...
public class PivnyStvrtokStateMachineConfig extends EnumStateMachineConfigurerAdapter<States, Events>{
    ...
    public void configure(StateMachineTransitionConfigurer<States, Events> transitions)
        ...
        transitions
            .withExternal()
                .source(States.INITIAL).target(States.VOTING).event(Events.CREATE)
                .and()
            .withExternal()
                .source(States.VOTING).target(States.VOTED).event(Events.UPDATE)
                .and()
            .withExternal()
                .source(States.VOTED).target(States.FINISHED).event(Events.UPDATE);
    }
